...
- **TASK_PRESETS Registry**: Automated hyperparameter grounding for standard benchmarks (XOR, Parity, ASCII Decoder).
- **Mini-Batch Training**: Introduced batch-sliced optimization to `step()` to support high-N tasks (ASCII) without WebGL memory overhead.

### Fixed

- **Phasic Parity**: Restored `spinMultiplier=2.0` (Double Cover) to align with Python UIT 1.3.12 reference.
- **Tensor Leaks**: Manual disposal of intermediate `hState` and `currentInput` tensors in temporal loops.
- **UI Constraints**: Expanded `Hidden` slider max from 16 to 1024 to support high-fidelity ASCII decoding benchmarks.
- **Visual Projection**: Corrected `getPosition` to normalize $4\pi$ spinor space into the $2\pi$ visual manifold.

---

## [1.3.12] - "Topological Refinement" - 2026-01-27

### Changed

- **Scientific Visualization Standard (Premium Suite):** Overhauled the plotting logic for all active `UIT_ROUND` benchmarks (`Crystalline Loop`, `Sandwich Duel`, `Prism Stack`, `Color Algebra`, `Sine Waves`) to meet academic "Scientific Protocol" standards.
  - **Advanced Three-Panel Diagnostic:** Standardized a 3-panel layout (Persistence Map | Survival Rate | Hypertorus Projection) for deep-core telemetry.
  - **Persistence Heatmaps (Panel A):** Implemented high-contrast, grid-aligned heatmaps using the "Forest Wall" palette (`forestgreen` for success, `maroon` for error) to visualize the 100% recall "Crystalline" threshold.
  - **Comparative Logic Planes (Panel C):** Refined the 512D $\to$ 2D manifold projection. Removed misleading unit-circle overlays; introduced "Logic Plane" scaling ($1.0 \times \text{GRU\_Acc}$) to visually demonstrate Euclidean collapse vs. Phasic Manifold exploration.
- **Aesthetic Hardening:** Enforced a unified Dark Mode palette (`#0A0B10`) and high-fidelity annotations (e.g., "GRU: Collapsed to origin") for direct compatibility with the ROUND White Paper (UID 0140).
- **Battery Orchestration:** Refactored `UIT_run_battery_targeted.py` to support granular path hygiene (`--output_dir`, `--log_dir`) and task-specific learning rate injection (e.g., $10^{-3}$ for Riemannian stability).

### Removed

- **Transparency Duel Elimination:** Permanently decommissioned the `Transparency Duel` benchmark. Empirical testing confirmed the metric provided a misleading representation of "transparency," as its findings were either redundant with the `Sandwich Duel` or masked by auxiliary "warp" layer learning.

## [1.3.11] - "Industrial Crystal" - 2025-12-31

### Added

- **Premium Visualization Suite:** Upgraded all active benchmarks (`crystalline_loop`, `sandwich_duel`, `u_matrix`, `prism_stack`) to use the "Sandwich Style" Dark Mode standard (Red/Blue/Green palettes, high-contrast text).
